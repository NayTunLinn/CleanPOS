...
export function DashboardView() {
  ...
  const stats = [
    ...
    { label: 'Avg Order', value: formatCurrency(avgOrder), icon: ArrowUpRight, accent: 'bg-slate-100 text-slate-600 dark:bg-zinc-800 dark:text-zinc-400', ring: 'ring-slate-200 dark:ring-zinc-700' },
  ];
    ...
    <div className="h-full overflow-y-auto bg-slate-50 p-6 dark:bg-zinc-950">
      <div className="mb-7 flex items-center justify-between">
        <div>
          ...
          <p className="mt-0.5 text-sm font-medium text-slate-400 dark:text-zinc-500">Sales overview and performance</p>
        </div>
        <div className="hidden items-center gap-2 rounded-xl bg-white px-4 py-2.5 shadow-soft dark:bg-zinc-900 dark:shadow-none sm:flex">
          <Zap size={16} className="text-brand-500" />
          <span className="text-xs font-semibold text-slate-600 dark:text-zinc-300">Live data</span>
      ...
      <div className="mt-6 grid grid-cols-1 gap-4 lg:grid-cols-3">
        <Card className="p-5 lg:col-span-2">
          <div className="mb-5 flex items-center justify-between">
            ...
            <span className="badge bg-slate-100 text-slate-500 dark:bg-zinc-800 dark:text-zinc-400">By Revenue</span>
          </div>
          {topProducts.length === 0 ? (
            <div className="flex h-40 flex-col items-center justify-center text-center">
              <TrendingUp size={32} className="mb-2 text-slate-200 dark:text-zinc-700" />
              <p className="text-sm font-medium text-slate-400 dark:text-zinc-500">No sales yet — complete an order to see data</p>
            </div>
          ) : (
            <div className="space-y-3.5">
              {topProducts.map((p, idx) => (
                <div key={p.name} className="flex items-center gap-3">
                  <span className="flex h-6 w-6 items-center justify-center rounded-lg bg-slate-100 text-xs font-bold text-slate-500 dark:bg-zinc-800 dark:text-zinc-400">{idx + 1}</span>
                  <span className="text-2xl">{p.emoji}</span>
                  <div className="flex-1">
                    <div className="flex justify-between text-sm">
                      <span className="font-semibold text-slate-900 dark:text-white">{p.name}</span>
                      <span className="font-bold text-slate-700 dark:text-zinc-300">{formatCurrency(p.revenue)}</span>
                    </div>
                    <div className="mt-1.5 h-2 overflow-hidden rounded-full bg-slate-100 dark:bg-zinc-800">
                      <div
                  ...
                  <span className="w-14 text-right text-xs font-medium text-slate-400 dark:text-zinc-500">{p.qty} sold</span>
                </div>
              ))}
            </div>
          )}
        ...
        <Card className="p-5">
          ...
          {lowStock.length === 0 ? (
            <div className="flex h-40 flex-col items-center justify-center text-center">
              <Package size={32} className="mb-2 text-slate-200 dark:text-zinc-700" />
              <p className="text-sm font-medium text-slate-400 dark:text-zinc-500">All products well stocked</p>
            </div>
          ) : (
            <div className="space-y-2">
              {lowStock.map((p) => (
                <div key={p.id} className="flex items-center gap-3 rounded-xl border border-slate-100 bg-slate-50/60 p-2.5 transition hover:border-slate-200 dark:border-zinc-800 dark:bg-zinc-800/50 dark:hover:border-zinc-700">
      ...
      <Card className="mt-6 p-5">
        ...
        {orders.length === 0 ? (
          <div className="flex h-40 flex-col items-center justify-center text-center">
            ...
            <p className="text-sm font-medium text-slate-400 dark:text-zinc-500">No orders yet</p>
          </div>
        ) : (
          <div className="space-y-2">
            {orders.slice(0, 6).map((o) => (
              <div key={o.id} className="flex items-center justify-between rounded-xl border border-slate-100 p-3 transition hover:border-slate-200 hover:bg-slate-50/50 dark:border-zinc-800 dark:hover:border-zinc-700 dark:hover:bg-zinc-800/50">
                <div className="flex items-center gap-3">
                  <span className="rounded-lg bg-slate-900 px-2.5 py-1 text-xs font-bold text-white dark:bg-zinc-100 dark:text-zinc-900">{o.id}</span>
                  <div>
                    <p className="text-sm font-semibold text-slate-900 dark:text-white">
                      {o.items.reduce((s, i) => s + i.quantity, 0)} items · <span className="capitalize">{o.paymentMethod}</span>
                    </p>
                    <p className="text-xs font-medium text-slate-400 dark:text-zinc-500">{formatTime(o.createdAt)}</p>
                  </div>
                </div>
