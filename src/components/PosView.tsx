  ...
  return (
    <div className="flex h-full overflow-hidden">
      <div className="flex flex-1 flex-col overflow-hidden">
        ...
        <div className="border-b border-slate-200/70 bg-white px-6 pb-4 pt-5 dark:border-slate-800 dark:bg-slate-900">
          <form onSubmit={handleScan} className="mb-3 flex items-center gap-3">
            <div className="relative flex-1">
              ...
              <input
                ...
                className="w-full rounded-xl border border-slate-200 bg-slate-50 py-2.5 pl-10 pr-4 text-sm text-slate-900 outline-none transition-all duration-200 focus:border-brand-500 focus:bg-white focus:shadow-glow-brand dark:border-slate-700 dark:bg-slate-800 dark:text-slate-100 dark:placeholder:text-slate-500 dark:focus:bg-slate-800"
              />
            </div>
        ...
        {/* Product grid */}
        <div className="flex-1 overflow-y-auto bg-slate-50 px-6 py-5 dark:bg-slate-950">
                ...
                <button
                  ...
                  className={cn(
                    ...
                    out
                      ...
                      : 'border-slate-200/70 shadow-soft hover:-translate-y-0.5 hover:border-brand-300 hover:shadow-soft-md active:scale-[.97] dark:border-slate-800 dark:hover:border-brand-700 dark:shadow-none',
                  )}
                >
                  {inCart && (
                  ...
                  <p className={cn('mt-0.5 text-[11px] font-medium', out ? 'text-red-500' : 'text-slate-400 dark:text-slate-500')}>
          ...
          {filtered.length === 0 && (
            <div className="flex h-48 flex-col items-center justify-center text-center">
              <Search size={32} className="mb-2 text-slate-300 dark:text-slate-600" />
      ...
      {/* Cart panel */}
      <div className="flex w-[380px] flex-col border-l border-slate-200/70 bg-white dark:border-slate-800 dark:bg-slate-900">
        <div className="flex items-center justify-between border-b border-slate-200/70 px-5 py-4 dark:border-slate-800">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <p className="text-xs font-medium text-slate-400 dark:text-slate-500">{cartCount} item{cartCount !== 1 ? 's' : ''}</p>
            </div>
          </div>
          {cart.length > 0 && (
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4">
          {cart.length === 0 ? (
            <div className="flex h-full flex-col items-center justify-center text-center">
              <div className="mb-3 flex h-16 w-16 items-center justify-center rounded-2xl bg-slate-100 text-slate-300 dark:bg-slate-800 dark:text-slate-600">
                <ShoppingCart size={28} />
              </div>
              <p className="text-sm font-semibold text-slate-400 dark:text-slate-500">Cart is empty</p>
              <p className="mt-1 text-xs text-slate-400 dark:text-slate-500">Tap products or scan a SKU to start</p>
            </div>
          ) : (
            <div className="space-y-2">
              {cart.map((item) => (
                <div
                  key={item.product.id}
                  className="flex animate-slide-in-right items-center gap-3 rounded-xl border border-slate-100 bg-slate-50/60 p-2.5 transition hover:border-slate-200 dark:border-slate-800 dark:bg-slate-800/60 dark:hover:border-slate-700"
                >
                  <span className="text-2xl">{item.product.emoji}</span>
                  <div className="flex-1 min-w-0">
                    <p className="truncate text-sm font-bold text-slate-900 dark:text-white">{item.product.name}</p>
                    <p className="text-xs font-medium text-slate-400 dark:text-slate-500">{formatCurrency(item.product.price)} each</p>
                  </div>
                  <div className="flex items-center gap-1">
                    <button
                      onClick={() => decrementFromCart(item.product.id)}
                      className="flex h-7 w-7 items-center justify-center rounded-lg bg-white text-slate-600 shadow-sm transition hover:bg-slate-100 active:scale-90 dark:bg-slate-700 dark:text-slate-300 dark:hover:bg-slate-600"
                    >
                      <Minus size={14} />
                    </button>
        ...
        <div className="border-t border-slate-200/70 px-5 py-4 dark:border-slate-800">
          <div className="space-y-1.5">
            ...
            <div className="flex justify-between border-t border-slate-100 pt-2.5 text-lg font-extrabold text-slate-900 dark:border-slate-800 dark:text-white">
              <span>Total</span>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 p-4 backdrop-blur-sm animate-fade-in dark:bg-black/60" onClick={onClose}>
      <Card className="w-[360px] animate-scale-in p-6" onClick={(e) => e.stopPropagation()}>
        <div className="flex flex-col items-center text-center">
          ...
          <p className="text-sm font-medium text-slate-400 dark:text-slate-500">Order {order.id}</p>
        </div>

        <div className="my-4 space-y-1.5 border-y border-slate-100 py-3 dark:border-slate-800">
        ...
        <div className="space-y-1 text-sm">
          ...
          <div className="flex justify-between pt-1 text-xs text-slate-400 dark:text-slate-500">
